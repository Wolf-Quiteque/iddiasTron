...
const SearchProfession = () => {
  ...
  const [name, setname] = React.useState(
    User.profession ? User.profession : "llllll"
  );
    ...
    try {
      const res = await fetch(
        "https://iddias-api-sehk.vercel.app/api/iddias/find",
        {
          ...
          body: JSON.stringify({
            profession: { $regex: ".*" + name + "*.", $options: "i" },
          }),
        }
      );
    ...
    <ScrollView style={{ backgroundColor: "white" }}>
      ...
      >
        <View style={[styles.navigationBarView, styles.ml80]}>
          <Text style={styles.searchName}>Search - Profession</Text>
          <Pressable
          ...
          users.map((u) => (
            <>
              {u._id != User._id && (
                <View
                  key={u._id}
                  style={[styles.nameView, styles.mt35, styles.mr1]}
                >
